...
<div class="dashboard-container">
    ...
    <div class="dashboard-content-container" data-simplebar>
        <div class="dashboard-content-inner">
                    ...
                    <div class="messages-inbox">
                        <div class="messages-headline">
                            <div class="input-with-icon">
                                <input id="conversation-search" type="text" placeholder="<?php echo _e('Search');?>">
                        ...
                        <ul id="conversations-wrapper">
                            <?php foreach ($conversations as $conversation) { ?>
                                <li>
                                    <a href="javascript:void(0)" class="conversation" data-id="<?php _esc($conversation['id']) ?>">
                                        <div class="message-by margin-left-0">
                                            <div class="message-by-headline">
                                                <h5><?php _e(_esc($conversation['title'])) ?></h5>
                                                <input class="conversation-title with-border small-input" type="text" value="<?php _esc(escape($conversation['title'])) ?>">
                                                <span class="conversation-time"><?php _esc(timeAgo($conversation['updated_at'])) ?></span>
                                                <span class="conversation-edit"><i class="icon-feather-edit"></i> <?php _e('Edit') ?></span>
                                            </div>
                        ...
                        <div class="messages-inbox-bottom">
                            <a href="javascript:void(0)" id="new-conversation" class="button full-width button-sliding-icon"><?php _e('New Conversation'); ?> <i class="icon-feather-plus"></i></a>
                        </div>
                    </div>
                    <!-- Messages / End -->
                    <!-- Message Content -->
                    <div class="message-content">
                        <div class="messages-headline">
                            <h4 class="d-flex align-items-center">
                                <div class="user-avatar margin-right-10">
                                    <img src="<?php _esc($ai_chat_bot_avatar) ?>" alt="<?php _esc($ai_chat_bot_name) ?>" width="25" />
                                </div>
                            ...
                            <div class="message-action">
                                <a href="#" class="button ripple-effect btn-sm d-md-none d-inline-block" id="show-conversations" title="<?php _e("Show Conversations") ?>" data-tippy-placement="top"><i class="icon-feather-menu"></i></a>
                                <a href="#" class="button ripple-effect btn-sm" id="export-chats" title="<?php _e("Export Conversation") ?>" data-tippy-placement="top"><i class="icon-feather-download"></i></a>
                                <a href="#" class="button ripple-effect btn-sm red" id="delete-chats" title="<?php _e("Delete Conversation") ?>" data-tippy-placement="top"><i class="icon-feather-trash-2"></i></a>
                            </div>
                        </div>
                                ...
                                <div class="message-bubble">
                                    <div class="message-bubble-inner">
                                        <div class="message-avatar">
                                            <img src="<?php _esc($ai_chat_bot_avatar) ?>" alt="<?php _esc($ai_chat_bot_name) ?>" />
                                        </div>
                        ...
                        <form id="ai-chat-form">
                            <?php if (get_option("enable_ai_chat_mic", 1)) { ?>
                                <!-- <a id="chat-microphone-div" data-href="javascript:void(0)" style="display: flex; justify-content: center; width: 100%; background-color: var(--theme-color-0_1); cursor: pointer; align-items: center; "> -->
                                <a id="chat-microphone" style="display: flex; justify-content: center; width: 100%; background-color: var(--theme-color-0_1); cursor: pointer; align-items: center; " data-href="javascript:void(0)"><i class="fa fa-microphone"></i></a>
                                <!-- </a> -->
                            <?php } ?>
                            <div class="message-reply">
                                <input type="hidden" name="bot_id" id="bot_id" value="<?php _esc($bot_id) ?>">
                                <textarea placeholder="<?php _e('Type your message here (Use Enter to send)') ?>" id="ai-chat-textarea" rows="2"></textarea>
                                <div>
                                    <?php if (get_option("enable_ai_chat_prompts", 1)) { ?>
                                        <a id="chat-prompts" href="javascript:void(0)" title="<?php _e('Prompt Library'); ?>" data-tippy-placement="top"><i class="fa fa-book"></i></a>
                                    <?php } ?>
                                    <!-- <?php if (get_option("enable_ai_chat_mic", 1)) { ?>
                                        <a id="chat-microphone" href="javascript:void(0)"><i class="fa fa-microphone"></i></a>
                                    <?php } ?> -->
                                    <button id="chat-send-button" type="submit" class="button ripple-effect">
                                        <?php _e('Send') ?>
                                    </button>
                                </div>
                            </div>
                            <div style="margin-left: 20px; margin-right: 20px;" class="row">
                                <div class="col-sm-3">
                                    <div class="submit-field margin-bottom-20">
                                        <h6><?php _e("Write in") ?></h6>
                                        <select name="language_write" id="language_write" class="with-border small-input" required>
                                            <?php foreach (get_ai_chat_writein() as $language) { ?>
                                                <option value="<?php _esc($language) ?>" <?php if ($language == 'English (USA)') echo 'selected'; ?>><?php _esc($language) ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <h6><?php _e("Tone") ?></h6>
                                    <select id="ai_chat_tone" name="ai_chat_tone" class="with-border small-input" required>
                                        <?php foreach (get_ai_chat_tone() as $tone) { ?>
                                            <option value="<?php _esc($tone) ?>" <?php if ($tone == 'Cynical') echo 'selected'; ?>><?php _esc($tone) ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="col-sm-3">
                                    <h6><?php _e("Responsive Style") ?></h6>
                                    <select id="responsive_id" name="responsive_id" class="with-border small-input" required>
                                        <?php foreach (get_ai_chat_responsive() as $responsive) { ?>
                                            <option value="<?php _esc($responsive) ?>" <?php if ($responsive == 'Conversational') echo 'selected'; ?>><?php _esc($responsive) ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="col-sm-3">
                                    <h6><?php _e("DFY tasks") ?></h6>
                                    <select id="ai_chat_dfy_task" name="ai_chat_dfy_task" class="with-border small-input" required>
                                        <?php foreach (get_ai_chat_dfy() as $dfy) { ?>
                                            <option value="<?php _esc($dfy) ?>"><?php _esc($dfy) ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
            ...
            <div style="text-align: center; margin-top: 20px;">AITeamUP app: Agents might sprinkle in a pinch of inaccuracy or a dash of unexpected humor (or even something offensive—definitely not our intention and style).</div> 
            <div style="text-align: center; margin-top: 20px;"> Your response is in the oven, cooking up. Be patient, and get ready for some AI charm!</div>
        ...
        <div class="sign-in-form">
            ...
            <div class="popup-tabs-container">
                ...
                <div class="popup-tab-content">
                    <input id="prompt-search" type="search" placeholder="<?php _e('Search Prompts') ?>" class="with-border">
                    <div id="chat-prompts-list">
                        ...
                        ?>
                            <a href="javascript:void(0)" data-search-key="<?php _esc(escape($chat_prompt['prompt'] . ' ' . $description)) ?>" data-prompt="<?php _esc(escape($chat_prompt['prompt'])) ?>">
