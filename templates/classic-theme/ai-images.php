        ...
        <div class="dashboard-content-inner">
            ...
            <form id="ai_images" name="ai_images" method="post" action="#">
                ...
                <div class="message-reply ai_image_description margin-bottom-10">
                    <input type="text" name="description" class="image-description" placeholder="<?php _esc($placeholders[array_rand($placeholders)]) ?>" required autocomplete="off" />
                    <button type="submit" name="submit" class="button ripple-effect border-pilled"><?php _e("Generate") ?>
                ...
                <div class="row image-advance-settings" style="display: none">
                    <div class="col-sm-3">
                        <div class="submit-field margin-bottom-20">
                            ...
                            <input name="title" class="with-border small-input" type="text" value="<?php _e("New Image") ?>">
                        </div>
                    </div>
                    <div class="col-sm-3">
                        <div class="submit-field margin-bottom-20">
                            <h6><?php _e("Art style") ?></h6>
                            <select name="style" id="style" class="with-border small-input selectpicker">
                    ...
                    <?php if (get_option('ai_image_api') != 'stable-diffusion' && check_allow()) { ?>
                        <div class="col-sm-3">
                            <div class="submit-field margin-bottom-20">
                                ...
                                <select name="resolution" id="resolution" class="with-border small-input selectpicker" required>
                    ...
                    <div class="col-sm-3">
                        <div class="submit-field margin-bottom-20">
                            ...
                            <select name="no_of_images" id="results" class="with-border small-input selectpicker" required>
            ...
            <div class="messages-container margin-top-0">
                <div class="messages-container-inner">
                    <!-- Messages -->
                    <div class="messages-inbox ai-tags-box">
                        <div class="messages-headline">
                            <div class="input-with-icon">
                                <input id="conversation-search" class="ai-tag-search" type="text" placeholder="Search">
                                <i class="icon-material-outline-search"></i>
                            </div>
                        </div>
                        <ul id="conversations-wrapper" class="ai-img-tag ai-tag-wrapper">
                            <?php if (count($ai_tags) > 0) {
                                foreach ($ai_tags as $key => $tag) {
                                    if (!empty($tag['tag'])) {
                            ?>
                                        <li class="<?php
                                                    if (!empty($active_tag) && !empty($active_tag === $tag['tag'])) {
                                                        echo 'active-message';
                                                    } else if ((empty($active_tag)) && ($key == 0)) {
                                                        echo 'active-message';
                                                    } ?> ai-tag-li">
                                            <a href="javascript:void(0)" class="conversation ai-tag-a <?php if ($key == 0) echo 'active'; ?>" data-id="<?php _esc($tag['id']) ?>">
                                                <div class="message-by margin-left-0">
                                                    <div class="message-by-headline">
                                                        <h5 id="tag-h5-<?= $tag['id']; ?>"><?= (strlen($tag['tag']) > 35) ? ucfirst(substr($tag['tag'], 0, 35 - 3) . '.....') : ucfirst($tag['tag']); ?></h5>
                                                        <input class="conversation-title ai-tag-title with-border small-input" type="text" value="<?php _esc($tag['tag']) ?>">
                                                        <span class="ai-tag-edit conversation-edit"><i class="icon-feather-edit"></i> <?php _e('Edit') ?></span>
                                                    </div>
                                                </div>
                                            </a>
                                        </li>
                        ...
                        <div class="messages-inbox-bottom ai-new-tag-buttom">
                            <a href="javascript:void(0)" id="new-tag-create" class="button full-width button-sliding-icon"><?php _e('Create New Tag'); ?> <i class="icon-feather-plus"></i></a>
                        </div>
                    </div>
                        ...
                        <div class="messages-headline">
                            <h4 class="d-flex align-items-center">
                                <div class="line-height-1">
                                    <!-- <span id="tag-section-name"><?php //_esc('AI Images - '. $active_tag_name) 
                                                                        ?></span> -->
                                    <span id="tag-section-name"><?php _e('AI Images') ?></span>
                                </div>
                            </h4>
                            <div class="message-action">
                                <a href="javascript:void(0)" class="button ripple-effect btn-sm red" id="delete-tag" title="<?php _e("Delete Conversation") ?>" data-tippy-placement="top"><i class="icon-feather-trash-2"></i></a>
                            </div>
                        </div>
                        <div class="message-content-inner">
                            <div class="row margin-top-25 margin-bottom-25 image-lightbox margin-0-auto" id="generated_images_wrapper">
                                <?php foreach ($ai_images as $ai_image) {
                                    if (!empty($ai_image['image'])) {
                                ?>
                                        <div class="col-sm-4 col-md-2 col-6 margin-bottom-30 icon_arrow" id="img-div-<?php echo _esc($ai_image['id']); ?>">
                                            <a href="<?php echo _esc($config['site_url'], 0) . 'storage/ai_images/' . $ai_image['image']; ?>" target="_blank" class="ai-lightbox-image" data-sub-html="<?php _esc($ai_image['description']) ?>">
                                                <img width="100%" src="<?php echo _esc($config['site_url'], 0) . 'storage/ai_images/' . $ai_image['image']; ?>" alt="<?php _esc($ai_image['title']) ?>" data-tippy-placement="top" title="<?php _esc($ai_image['description']) ?>" class="block-board-img">
                                            </a>
                                            <div class="overlay delete-block-img" data-id="<?php echo _esc($ai_image['id']); ?>">
...
<script src="<?php _esc(TEMPLATE_URL); ?>/js/ai-images.js?ver=<?php _esc($config['version']); ?>"></script>
<script>
    ...
    const LANG_NEW_TAG_CREATE = "<?php _e('New Tag') ?>";
</script>
