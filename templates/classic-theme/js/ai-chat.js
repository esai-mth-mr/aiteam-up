(function ($) {
  ...
  $(document).on("click", ".conversation", function (e) {
    ...
    if ($this.closest("li").hasClass("active-message")) return;
    ...
    if ($conv_id === "default") formData.append("bot_id", $bot_id.val());
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          for (const chat of response.chats) {
            ...
            if (days) {
              $msgChat.append(
                '<div class="message-time-sign"><span>' +
                  chat.date_formatted +
                  "</span></div>"
              );
            }
  ...
  $("#new-conversation").on("click", function (e) {
    ...
    formData.set("conv_name", "New Conversation");
    formData.set("bot_id", $bot_id.val());

    $.ajax({
      type: "POST",
      url: ajaxurl + "?action=create_new_conversation",
      data: formData,
      dataType: "json",
      cache: false,
      contentType: false,
      ...
      success: function (response) {
        if (response.success) {
          $(".conversation.active").data("id", response.conv_id);
          $(".conversation.active .message-by-headline")
            .html(`<h5 style="">New Conversation</h5>
              <input class="conversation-title with-border small-input" type="text" value="New Conversation" style="display: none;" data-listener-added_562ecb70="true">
  ...
  $(document)
    .on("click", ".conversation-edit", function (e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  function update_title() {
    ...
    if ($name.text() != value) {
      $edit_icon
        .addClass("button-loader button-loader-dark")
        .prop("disabled", true);
      $.post(ajaxurl + "?action=edit_conversation_title", data, function () {
        $edit_icon
          .removeClass("button-loader button-loader-dark")
          .prop("disabled", false);
      });
    }
  ...
  $("#export-chats").on("click", function (e) {
    ...
    $.ajax({
      ...
      url:
        ajaxurl +
        "?action=export_ai_chats&bot_id=" +
        $bot_id.val() +
        "&conv_id=" +
        $(".conversation.active").data("id"),
      dataType: "json",
      success: function (response) {
        $btn.removeClass("button-progress").prop("disabled", false);
        if (response.success) {
          ...
          downloadableLink.setAttribute(
            "href",
            "data:text/plain;charset=utf-8," + encodeURIComponent(response.text)
          );
      ...
      .each(function () {
        if (
          $(this).filter(function () {
            return (
              $(this).find("h5").text().toLowerCase().indexOf(searchTerm) > -1
            );
          }).length > 0 ||
          searchTerm.length < 1
        ) {
  ...
  $form.on("submit", function (e) {
    ...
    let write_in = document.getElementById('language_write').value;
    let tone = document.getElementById('ai_chat_tone').value;  
    let responsive_style = document.getElementById('responsive_id').value;  
    let dfy_task = document.getElementById('ai_chat_dfy_task').value;  

    var formData = new FormData();
    formData.append("msg", msgText);
    formData.append("bot_id", $bot_id.val());
    formData.append("conv_id", $(".conversation.active").data("id"));
    formData.append("write_in", write_in);
    formData.append("tone", tone);
    formData.append("responsive_style", responsive_style);
    formData.append("dfy_task", dfy_task);
    ...
    $.ajax({
      ...
      success: async function (response) {
        if (response.success) {
          ...
          const text = escape_html(response.ai_message);
          ...
          await simulateTyping(response.ai_message, 0);

          let $active_conversation = $(".conversation.active");
          $active_conversation.data("id", response.conversation_id);

          eventSource = new EventSource(
            `${ajaxurl}?action=chat_stream&conv_id=${
              response.conversation_id
            }&bot_id=${$bot_id.val()}&last_message_id=${
          ...
          eventSource.onmessage = function (e) {
            if (e.data === "[DONE]") {
              ...
              $active_conversation
                .find(".conversation-msg")
                .text(msg.substring(0, 99));
    ...
    $("#chat-prompts-list")
      ...
      .each(function () {
        if (
          $(this).filter(function () {
            return (
              $(this).data("search-key").toLowerCase().indexOf(searchTerm) > -1
            );
          }).length > 0 ||
          searchTerm.length < 1
        ) {
  ...
  function appendMessage(name, img, side, text, id) {
    ...
    let typing =
      text.length === 0 && side === "left"
        ...
        : "";
    let msg_show =
      text.length === 0 && side === "left" ? 'style="display:none"' : "";
