...
if (isset($_REQUEST['action'])) {
    ...
    if ($_REQUEST['action'] == "deleteMembershipPlan") {
        deleteMembershipPlan();
    }
    if ($_REQUEST['action'] == "deletePrepaidPlan") {
        deletePrepaidPlan();
    }
    if ($_REQUEST['action'] == "deleteTaxes") {
        deleteTaxes();
    }
    if ($_REQUEST['action'] == "deleteTransaction") {
        deleteTransaction();
    }
    if ($_REQUEST['action'] == "deleteCurrency") {
        deleteCurrency();
    }
    if ($_REQUEST['action'] == "deleteTimezone") {
        deleteTimezone();
    }
    if ($_REQUEST['action'] == "blogCatPosition") {
        blogCatPosition();
    }
    if ($_REQUEST['action'] == "installPayment") {
        installPayment();
    }
    if ($_REQUEST['action'] == "uninstallPayment") {
        uninstallPayment();
    }
    if ($_REQUEST['action'] == "deleteTestimonial") {
        deleteTestimonial();
    }
    if ($_REQUEST['action'] == "deleteLanguage") {
        deleteLanguage();
    }
    if ($_REQUEST['action'] == "deleteSubscriber") {
        deleteSubscriber();
    }
    if ($_REQUEST['action'] == "deleteusers") {
        deleteUsers();
    }
    if ($_REQUEST['action'] == "deleteAdmin") {
        deleteAdmin();
    }
    if ($_REQUEST['action'] == "deleteStaticPage") {
        deleteStaticPage();
    }
    if ($_REQUEST['action'] == "deletefaq") {
        deletefaq();
    }
    if ($_REQUEST['action'] == "addPlanCustom") {
        addPlanCustom();
    }
    if ($_REQUEST['action'] == "editPlanCustom") {
        editPlanCustom();
    }
    if ($_REQUEST['action'] == "delPlanCustom") {
        delPlanCustom();
    }
    if ($_REQUEST['action'] == "langTranslation_PlanCustom") {
        langTranslation_PlanCustom();
    }
    if ($_REQUEST['action'] == "edit_langTranslation_PlanCustom") {
        edit_langTranslation_PlanCustom();
    }
    if ($_REQUEST['action'] == "edit_langTranslation") {
        edit_langTranslation();
    }
    if ($_REQUEST['action'] == "langTranslation_FormFields") {
        langTranslation_FormFields();
    }
    if ($_REQUEST['action'] == "editLanguageFile") {
        editLanguageFile();
    }
    if ($_REQUEST['action'] == "saveBlog") {
        saveBlog();
    }
    if ($_REQUEST['action'] == "deleteBlog") {
        deleteBlog();
    }
    ...
    if ($_REQUEST['action'] == "delBlogCat") {
        delBlogCat();
    }

    if ($_REQUEST['action'] == "quickad_update_maincat_position") {
        quickad_update_maincat_position();
    }
    if ($_REQUEST['action'] == "quickad_update_subcat_position") {
        quickad_update_subcat_position();
    }
    if ($_REQUEST['action'] == "quickad_update_plan_custom_position") {
        quickad_update_plan_custom_position();
    }
    if ($_REQUEST['action'] == "aiTplCategoryPosition") {
        aiTplCategoryPosition();
    }
    if ($_REQUEST['action'] == "aiChatBotCategoryPosition") {
        aiChatBotCategoryPosition();
    }
    if ($_REQUEST['action'] == "aiChatBotsPosition") {
        aiChatBotsPosition();
    }
    if ($_REQUEST['action'] == "aiChatPromptsPosition") {
        aiChatPromptsPosition();
    }
    if ($_REQUEST['action'] == "aiTplPosition") {
        aiTplPosition();
    }
    if ($_REQUEST['action'] == "prepaidPlansPosition") {
        prepaidPlansPosition();
    }
    if ($_REQUEST['action'] == "plansPosition") {
        plansPosition();
    }
    if ($_REQUEST['action'] == "paymentMethodsPosition") {
        paymentMethodsPosition();
    }

    if ($_REQUEST['action'] == "deleteAIDocument") {
        deleteAIDocument();
    }
    if ($_REQUEST['action'] == "deleteAIImages") {
        deleteAIImages();
    }
    if ($_REQUEST['action'] == "deleteAICustomTemplates") {
        deleteAICustomTemplates();
    }
    if ($_REQUEST['action'] == "deleteAITplCategories") {
        deleteAITplCategories();
    }
    if ($_REQUEST['action'] == "deleteAPIKeys") {
        deleteAPIKeys();
    }
    if ($_REQUEST['action'] == "deleteAIChatBots") {
        deleteAIChatBots();
    }
    if ($_REQUEST['action'] == "deleteAIChatBotsCategories") {
        deleteAIChatBotsCategories();
    }
    if ($_REQUEST['action'] == "deleteAIChatPrompts") {
        deleteAIChatPrompts();
    }
    if ($_REQUEST['action'] == "deleteAISpeeches") {
        deleteAISpeeches();
    }
    if ($_REQUEST['action'] == "send_ticket_message") {
        send_ticket_message_admin();
    }
}

function send_ticket_message_admin()
{
    $result = array();
    global $config;
    ...
    send_admin_message_notification($_POST['ticket_id'], $user_email, $ticket_title, $_POST['message']);
