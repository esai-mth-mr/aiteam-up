(function ($) {
  ...
  $(document).on("ready", function () {
    ...
    quick_load_css(
      "quick-google-font",
      "//fonts.googleapis.com/css?family=Nunito:400,400i,600,600i,700,700i,800,800i&display=swap"
    );
  });
  ...
  $(".quick-ripple-effect, .quick-ripple-effect-dark").on(
    "click",
    function (e) {
      var rippleDiv = $('<span class="quick-ripple-overlay">'),
  ...
  $(document)
    .on("change", "#quick-checkbox-all", function () {
      $(".quick-check")
  ...
  if ($(".tiny-editor").length) {
    tinymce.init({
      ...
      resize: true,
      plugins:
        "quickbars image advlist lists code table codesample autolink link wordcount fullscreen help searchreplace media anchor",
      toolbar: [
      ...
      link_class_list: [
        { title: "None", value: "" },
        {
          ...
          value: "btn btn-sm btn-primary shadow-primary",
        },
        {
          title: "Secondary Button",
          value: "btn btn-sm btn-secondary shadow-secondary",
        },
        {
          title: "Danger Button",
          value: "btn btn-sm btn-danger shadow-danger",
        },
        {
          title: "Warning Button",
          value: "btn btn-sm btn-warning shadow-warning",
        },
  ...
  $(document)
    .off("click", "[data-toggle=slidePanel]")
    .on("click", "[data-toggle=slidePanel]", function (e) {
      e.stopPropagation();
      e.preventDefault();

      var $btn = $(this);
      $.slidePanel.show(
        { url: $(this).data("url"), settings: { cache: false } },
        {
...
$(document).on("click", "#post_sidePanel_ticket_data", function (e) {
  var $button = $(this);
  $form = $("#ticketPanel_form");
  const action = $form.data("ajax-action");

  const status = document.getElementById("admin_status_option").value;
  const category = document.getElementById("admin_category_option").value;
  const priority = document.getElementById("admin_priority_option").value;
  const ticket_id = document.getElementById("id_ticket_id").value;
    ...
    success: function (response) {
      if (response == 0) {
        ...
        //   quick_alert(response.message);
          ...
          document.getElementById(ticket_id).childNodes[7].innerHTML = category;
          
          $(document.getElementById(ticket_id).childNodes[8].childNodes[0]).attr('class', 'badge');
          document.getElementById(ticket_id).childNodes[8].childNodes[0].classList.add("priority-" + priority.toLowerCase());
          document.getElementById(ticket_id).childNodes[8].childNodes[0].innerHTML = priority;

          if(status == "open") {
            $(document.getElementById(ticket_id).childNodes[9].childNodes[0]).attr('class', 'badge');
            document.getElementById(ticket_id).childNodes[9].childNodes[0].classList.add("badge-warning");
            document.getElementById(ticket_id).childNodes[9].childNodes[0].innerHTML = "open";
          } else if(status == "progress") {
            $(document.getElementById(ticket_id).childNodes[9].childNodes[0]).attr('class', 'badge');
            document.getElementById(ticket_id).childNodes[9].childNodes[0].classList.add("badge-info");
            document.getElementById(ticket_id).childNodes[9].childNodes[0].innerHTML = "progress";
          } else {
            $(document.getElementById(ticket_id).childNodes[9].childNodes[0]).attr('class', 'badge');
            document.getElementById(ticket_id).childNodes[9].childNodes[0].classList.add("badge-success");
            document.getElementById(ticket_id).childNodes[9].childNodes[0].innerHTML = "resolved";
          }
        } else {
...
function support_send_message (id) {
    ...
    success: function (response) {
      if (response.success) {
        ...
        side = "me";
          ...
          $("#ticket-view-chat-history-admin").append(msgHTML);
...
function quick_alert(message, type) {
  if (message) {
    ...
    if ($container.length == 0) {
      $container = jQuery('<div class="quick-alert-area"></div>').appendTo(
        "body"
      );
    }
    ...
    var $alert = jQuery(
      '<div role="alert" class="alert">' +
        '            <div class="d-flex">\n' +
        '                <i class="quick-alert-icon ' +
        class_name +
        '"></i>' +
        '                <div class="quick-alert-text">' +
        '                    <h2 class="quick-alert-title">' +
        alert_type +
        "</h2>" +
        '                    <p class="quick-alert-message">' +
        message +
        "</p>" +
    ...
    $alert
      .appendTo($container)
      .fadeIn()
      .css("transform", "translate3d(0%, 0px, 0px)");
