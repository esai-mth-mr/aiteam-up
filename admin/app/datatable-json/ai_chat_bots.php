...
// Where condition to get agent list which is created by the admin, not by the users
$where = " WHERE cb.is_uai_agent = 0 ";

// check search value exist
if (!empty($params['search']['value'])) {
    // $where .= " WHERE ";
    $where .= " AND ( cb.name LIKE '%" . $params['search']['value'] . "%' ";
...
foreach ($queryRecords as $row) {
    ...
    if(!empty($row['image']))
        $img_url = $config['site_url'].'storage/bot_images/'.$row['image'];
    else
