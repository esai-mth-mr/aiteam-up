    ...
    <div class="slidePanel-inner">
        <form method="post" id="sidePanel_form" data-ajax-action="editMembershipPlan">
            ...
            <div class="form-body">
                ...
                <div class="form-group">
                    <label for="user_permissions"><?php _e('Features') ?></label>
                    <select class="form-control quick-multi-select" id="user_permissions" name="user_permissions[]" multiple>
                        <?php
                        $allPermissions = !empty($settings['user_permissions'])?$settings['user_permissions']:[];
                        $userPermissions = getUserSidebarMenus();
                        foreach ($userPermissions as $slug=>$userPermission) { 
                            echo '<option value="' . $slug . '" ' . (in_array($slug, $allPermissions) ? 'selected' : '') . '>' . $userPermission['title'] . '</option>';                 
                        }
                        ?>
                    </select>
                ...
                <div class="form-group">
                    <label for="ai_text_to_speech_limit"><?php _e('AI Voiceover Characters Per Month') ?></label>
                    ...
                    <span class="form-text text-muted"><?php _e('Set the characters limit for AI Voiceover. Set -1 for unlimited.') ?></span>
                    <?php if (!get_option('enable_text_to_speech')) { ?>
                        <small class="text-danger"><?php _e('AI Voiceover is disabled, please enable it from the AI settings to use it.'); ?></small>
                    <?php } ?>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input name="ai_speech_to_text_file_limit" type="number" class="form-control" id="ai_speech_to_text_file_limit" value="<?php _esc($settings['ai_speech_to_text_file_limit']) ?>">
                ...
                <div class="form-group">
                    ...
                    <input name="ai_words_limit" type="number" class="form-control" id="ai_words_limit" value="<?php _esc($settings['ai_words_limit']) ?>">
                    <span class="form-text text-muted"><?php _e('Set -1 for unlimited.') ?></span>
                </div>
                <div class="form-group">
                    <label for="ai_images_limit"><?php _e('AI Images Per Month') ?></label>
                    <input name="ai_images_limit" type="number" class="form-control" id="ai_images_limit" value="<?php _esc($settings['ai_images_limit']) ?>">
                    <span class="form-text text-muted"><?php _e('Set -1 for unlimited.') ?></span>
                </div>
                <div class="form-group">
                    <label for="ai_uai_agent_limit"><?php _e('UAI Agent Per Month') ?></label>
                    <input name="ai_uai_agent_limit" type="number" class="form-control" id="ai_uai_agent_limit" value="<?php _esc($settings['ai_uai_agent_limit']) ?>">
                    <span class="form-text text-muted"><?php _e('Set -1 for unlimited.') ?></span>
                </div>
                <div class="form-group">
                    <label for="ai_voices_limit"><?php _e('AI Voices') ?></label>
                    <input name="ai_voices_limit" type="number" class="form-control" id="ai_voices_limit" value="<?php _esc($settings['ai_voices_limit']) ?>">
                    <span class="form-text text-muted"><?php _e('Set -1 for unlimited.') ?></span>
                </div>
                <?php quick_switch(__('Analytics'), 'analytics', $settings['analytics'], __('Show analytics to the users')); ?>
