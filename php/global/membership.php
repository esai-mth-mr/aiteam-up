...
if(isset($current_user['id']))
{
    if( (isset($_REQUEST['upgrade'])) && (!empty($_REQUEST['upgrade'])) )
    {
        ...
        if(isset($_POST['payment_method_id']) && $_POST['payment_method_id'] != "")
        {
            ...
            {
                $userData = get_user_data('', $_SESSION['user']['id']);
                $access_token = uniqid();
                $_SESSION['quickad'][$access_token]['email'] = $userData['email'];
                ...
                $_SESSION['quickad'][$access_token]['free_plan'] = (!empty($userData['plan_type'])) ? 0 : 1; // 0 => already taken membership plan, 1 => first time subscription with free trail
        ...
        else
        {
            ...
            if($_REQUEST['upgrade'] == 'trial'){
                ...
                elseif($_REQUEST['billed-type'] == "annually") {
                    $period = 31536000;
                }
            }
            ...
            $expiry_date = $period ? date("m-d-Y",$expires) : __("Lifetime");
    ...
    else
	{
		...
		{            
            ...
            if($plan['status']){
                ...
                $sub_types[$plan['id']]['ai_uai_agent_limit'] = ($settings['ai_uai_agent_limit'] == -1)? __("Unlimited"): $settings['ai_uai_agent_limit'];
                $sub_types[$plan['id']]['ai_voices_limit'] = ($settings['ai_voices_limit'] == -1)? __("Unlimited"): $settings['ai_voices_limit'];
                $sub_types[$plan['id']]['analytics'] = $settings['analytics'];
...
{
    headerRedirect($link['TEMP_PRICING']);
}