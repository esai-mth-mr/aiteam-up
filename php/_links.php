<?php
global $config;
$link['HOME'] = $config['site_url'];
$link['INDEX'] = $config['site_url'];
$link['HOME'] = $config['site_url'] . 'home';
$link['DASHBOARD'] = $config['site_url'] . 'dashboard';
$link['LOGIN'] = $config['site_url'] . 'login';
$link['LOGOUT'] = $config['site_url'] . 'logout';
$link['SIGNUP'] = $config['site_url'] . 'signup';
$link['FORGOT'] = $config['site_url'] . 'forgot';
$link['REPORT'] = $config['site_url'] . 'report';
$link['CONTACT'] = $config['site_url'] . 'contact';
$link['SITEMAP'] = $config['site_url'] . 'sitemap';
$link['FAQ'] = $config['site_url'] . 'faq';
$link['FEEDBACK'] = $config['site_url'] . 'feedback';
$link['COUNTRIES'] = $config['site_url'] . 'countries';
$link['HTML'] = $config['site_url'] . 'html';
$link['BLOG'] = $config['site_url'] . 'blog';
$link['BLOG-SINGLE'] = $config['site_url'] . 'blog';
$link['BLOG-CAT'] = $config['site_url'] . 'blog/category';
$link['BLOG-AUTHOR'] = $config['site_url'] . 'blog/author';
$link['TESTIMONIALS'] = $config['site_url'] . 'testimonials';
$link['MESSAGE'] = $config['site_url'] . 'message';
$link['MEMBERSHIP'] = $config['site_url'] . 'membership';
$link['PAYMENT'] = $config['site_url'] . 'payment';
$link['IPN'] = $config['site_url'] . 'ipn';
$link['INVOICE'] = $config['site_url'] . 'invoice';
$link['TRANSACTION'] = $config['site_url'] . 'transaction';
$link['ADVERTISE_HERE'] = $config['site_url'] . 'advertise-here';
$link['ACCOUNT_SETTING'] = $config['site_url'] . 'account-setting';
$link['AFFILIATE-PROGRAM'] = $config['site_url'] . 'affiliate-program';
$link['WITHDRAWALS'] = $config['site_url'] . 'withdrawals';
$link['IMAGE_CONTENT'] = $config['site_url'] . 'imagecontent';
/*App Links*/
$link['AI_TEMPLATES'] = $config['site_url'] . 'ai-templates';
$link['AI_IMAGES'] = $config['site_url'] . 'ai-images';
$link['AI_CHAT'] = $config['site_url'] . 'ai-chat';
$link['AI_CHAT_BOTS'] = $config['site_url'] . 'ai-chat-bots';
$link['AI_SPEECH_TEXT'] = $config['site_url'] . 'ai-speech-text';
$link['AI_CODE'] = $config['site_url'] . 'ai-code';
$link['ALL_DOCUMENTS'] = $config['site_url'] . 'all-documents';
$link['ALL_CHAT'] = $config['site_url'] . 'all-chat';
$link['ALL_IMAGES'] = $config['site_url'] . 'all-images';
$link['AI_TEXT_SPEECH'] = $config['site_url'] . 'ai-text-speech';
$link['ALL_SPEECHES'] = $config['site_url'] . 'all-speeches';
$link['ALL_TEMPLATES'] = $config['site_url'] . 'all-templates';
$link['ALL_SPEECHE_TO_TEXT'] = $config['site_url'] . 'all-speech-to-text';
$link['CUSTOM_SUPPORT'] = $config['site_url'] . 'custom-support';
$link['UAI'] = $config['site_url'] . 'uai';
$link['UAI_CHATHISTORY'] = $config['site_url'] . 'uai/chat-history';
$link['Daily_Live_Tech'] = $config['site_url'] . 'account-live-tech';
$link['AI_PLOYEES'] = $config['site_url'] . 'account-ai-ploy';
$link['TEMP_PRICING'] = $config['site_url'] . 'temp-pricing';
// $link['EMBED_CHAT'] = $config['site_url'] . 'embed-chat';
