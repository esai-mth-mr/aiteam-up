...
function get_user_data($username = null, $userid = null)
{
    ...
    if (isset($info['id'])) {
        ...
        $userinfo['plan_type'] = $info['plan_type'];
...
function send_forgot_email($to, $id)
{
    ...
    email_send($to, $config['admin_email'], $email_subject, $email_body);
}

function send_ticket_create_notification($ticket_id, $title, $message)
{
    ...
    $data = ORM::for_table($config['db']['pre'] . 'email_template')
        ->select_many('subject', 'message')
        ->where('keyword', 'support_ticket_created')
        ->find_one();
    ...
    $html = str_replace('{USER_FULLNAME}', $user_name, $html);
    $html = str_replace('{USER_EMAIL}', $user_email, $html);
    $email_body = $html;

    return email_send($config['admin_email'], $config['admin_email'], $email_subject, $email_body);

}

function send_message_notification($ticket_id, $title, $message)
{
    ...
    $email_subject = "User is sending message on Ticket";
...
function checkbrute($user_id)
{
    ...
    $num_rows = ORM::for_table($config['db']['pre'] . 'login_attempts')
        ->where_any_is(array(
            array('user_id' => $user_id, 'time' => $valid_attempts)
        ), array('time' => '>'))
        ->count();
