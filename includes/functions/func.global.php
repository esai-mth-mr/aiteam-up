...
function checkMembership()
{
    ...
    if ($user->plan_type == NULL || $user->plan_type == "free") {

        if (CURRENT_PAGE == 'global/membership/changeplan') {
            return;
        }
        if (CURRENT_PAGE == 'global/temp-pricing') {
            return;
        }
        if (CURRENT_PAGE == 'global/membership') {
            if ($page_state == "payment_method") {
                return;
            }
            return;
        }
        if (CURRENT_PAGE == 'global/signup') {
            return;
        }
        if (CURRENT_PAGE == 'global/login') {
            return;
        }
        if (CURRENT_PAGE == 'global/logout') {
            return;
        }
        if (CURRENT_PAGE == 'global/forgot') {
            return;
        }
        $redirect_url = $config['site_url'] . 'membership/changeplan';
...
function overall_header($page_title = '', $meta_desc = '', $meta_image = '', $meta_article = false)
{
    ...
    // if($page_title == "Upgrade Membership") {
    //     // HtmlTemplate::display('overall_header');
    //     return;
    // }
...
function update_option_value($option, $value)
{
    ...
    // Update the row in the options table with the matching option_name
    $saved_option = ORM::for_table($config['db']['pre'] . 'options')
        ->where('option_name', $option)->find_one();

    $saved_option->set('option_value', $value);
...
function message($heading, $message, $forward = '', $back = true)
{
    ...
    headerRedirect($config['site_url'] . 'dashboard');
    exit;
}
...
function get_html_pages()
{
    ...
    foreach ($result as $info) {
        ...
        $htmlPages[$info['id']]['category'] = $info['category'];
...
function get_avatar_url_by_name($name, $length = 1)
{
    ...
    return $base_url . "storage/bot_images/" . $name;
}
