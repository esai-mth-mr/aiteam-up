...
if($_SERVER['SERVER_NAME'] == 'localhost'){
     define("ROOTPATH", $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'AiTeamUP');
}else{
...
// $stripe_secret_key = "sk_test_H1cPVenOeUmjUDCo2GeV7e7A";
// // $stripe_webhook_key = "we_1OKFmFJ0QiuJQozZJdgHQBTQ";
// $stripe_webhook_key = "whsec_nHkCgyU9GGRJYd2uDuKk8g7nqVtqWYmY";
...
if(empty($siteUrl)){
    if($_SERVER['SERVER_NAME'] == 'localhost'){
        $siteUrl = 'http://localhost/aiteamup/';
    }else{
        $siteUrl = 'https://aiteamup.com/';
    }
}
if (substr($siteUrl, -1) !== '/') {
    $siteUrl = $siteUrl.'/';
...
switch ($event->type) {
    ...
    case 'checkout.session.completed':
        ...
        //$endpoint = 'https://aiteamup.com/php/awwhehgrjy.php?action=webhook_request_checkout_session_completed';
        $endpoint = $siteUrl.'php/awwhehgrjy.php?action=webhook_request_checkout_session_completed';

        $headers = array(
            'Content-Type: application/json', // Specify JSON content type
            'Content-Length: ' . strlen($json_data) // Specify the content length
        );
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $json_data); // Set the request data
...
if ($payment_type == "subscr") {
    ...
    if ($txn_type == 'subscr_update') {
        ...
        $user->plan_type = strtolower($payment_frequency);
        $user->save();        
